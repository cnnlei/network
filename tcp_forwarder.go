...
func handleTCPConnection(clientConn net.Conn, rule Rule, manager *ConnectionManager, ipFilterManager *IPFilterManager) {
	...
	if !ipFilterManager.IsAllowed(clientIP, rule.AccessControl) {
		log.Printf("[%s] 已拒绝来自 %s 的连接 (TCP 访问控制: %s '%s')", rule.Name, clientIP, rule.AccessControl.Mode, rule.AccessControl.ListName)
		clientConn.Close()
		return
	}
...
func startTCPForwarder(rule Rule, manager *ConnectionManager, ipFilterManager *IPFilterManager) {
	listener, err := net.Listen("tcp", rule.ListenAddress())
	if err != nil {
		log.Fatalf("致命错误: 无法为规则 [%s] 监听TCP端口 %s: %v", rule.Name, rule.ListenAddress(), err)
	}
	defer listener.Close()
	log.Printf("==== 规则 [%s] 已启动并成功监听TCP在 %s ====", rule.Name, rule.ListenAddress())
	for {
